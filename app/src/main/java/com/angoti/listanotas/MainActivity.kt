    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        serviceRetrofit.api?.getNotas()?.enqueue(object : Callback<List<Notas>> {
            override fun onResponse(call: Call<List<Notas>?>?, response: Response<List<Notas>?>?) {
                ...
                imageView1.setImageResource(if(listaDeNotas?.get(0)?.nota!! <6)R.drawable.vermelho else R.drawable.azul2)
                imageView2.setImageResource(if(listaDeNotas?.get(1)?.nota!! <6)R.drawable.vermelho else R.drawable.azul2)
                imageView3.setImageResource(if(listaDeNotas?.get(2)?.nota!! <6)R.drawable.vermelho else R.drawable.azul2)
